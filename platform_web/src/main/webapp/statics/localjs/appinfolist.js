$(function () {
    ...
    $("#queryFlatformId").change(function () {
        console.log(1)
        queryAppInfoListConditionForPageAnnCount(1, 5);
    })
    /**
     * 手游状态下拉列表选择变化事件
     */
    ...
    $("#lastBtn").on('click', function () {
        queryAppInfoListConditionForPageAnnCount($("#navigateLastPage").text(), 5)
    })
    /**
     * 上一页
     */
    $("#onBtn").on('click', function () {
        queryAppInfoListConditionForPageAnnCount($("#navigateFirstPage").text() - 1, 5)
    })
    /**
     * 下一页
     */
     ...
     * 上架按钮点击
     */
    $('#tableData').on("click", ".saleSwichOpen,.saleSwichClose", function () {
        let obj = $(this);
        console.log(obj);
     ...
     * 查看按钮
     */
    $("#tableData").on("click", '.viewApp', function () {
        var obj = $(this);
        window.location.href = "developer/toAppInfoView.do?appInfoId=" + obj.attr("appInfoId");
    });
    /**
     * 删除按钮
     */
    $("#tableData").on("click", '.deleteApp', function () {
        var obj = $(this);
        if (confirm("你确定要删除APP应用【" + obj.attr("appSoftwareName") + "】及其所有的版本吗？")) {
            $.ajax({
                type: "GET",
                url: "developer/deleteAppInfo.do",
 ...
 * @param pageSize
 */
function queryAppInfoListConditionForPageAnnCount(current, pageSize) {
    //收集参数
